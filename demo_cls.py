...
def _get_image_blob(im):
  ...
  processed_ims = []
  im_scale_factors = []
  if cfg.TEST.SCALES == 224:
    target_size = cfg.TEST.SCALES
    im_scale = None
    im = cv2.resize(im_orig, (target_size, target_size))#, fx=im_scale, fy=im_scale,
...
if __name__ == '__main__':
  ...
  if args.net == 'vgg16':
    ...
    fasterRCNN = pytorch_models.resnet101(pretrained=False)
    load_name = '/data/xiaodan/zengxiaohui/models/resnet101-5d3b4d8f.pth'
    import json
  ...
  #while (num_images >= 0):
  for num_images, (input, target) in enumerate(train_loader):
      total_tic = time.time()
      im_data = input.cuda()
      im_path, t = train_dataset.samples[num_images]
      print(im_path, t)
      #if webcam_num == -1:
      #  num_images -= 1
      ...
      if vis:
          ...
          #print(im2show)
          #exit()
      cls_scores = cls_scores.squeeze().softmax(0)
      #cls_scores = torch.
      scores, order = torch.sort(cls_scores, 0, True)
      ...
      if vis:
        ...
        for o in range(5):
            #print(order)
            j = order[o]
            print('top %d(%.2f) - %s'%(o, scores[o], classes[j]))
            im2show = vis_detections(im2show, classes[j], np.array([100,100*o,10,10, scores[0]]).reshape(1, 5), 0.5)
      ...
      if vis and webcam_num == -1:
          ...
          # cv2.waitKey(0)
          print(num_images)
          result_path = os.path.join('output', '%03d_det.jpg'%num_images) #imglist[num_images][:-4] + "_det.jpg")
